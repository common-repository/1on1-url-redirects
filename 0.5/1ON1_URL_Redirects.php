<?php
/*
Plugin Name: 1ON1 URL Redirects 
Description: 301 Redirects with Tags to post redirection option, which means when you view the tag it redirects to the assigned post or page of the tag which helps with SEO and resolves duplicate content issue.
Author: Marketing 1on1
Author URI: https://www.1on1.net
Version: 0.5
*/
...
class quick_page_post_reds {
	...
	function __construct() {
        ...
        add_action('wp_head',array($this,'redirect_tag_to_post'));			// parse query vars
		...
		add_action( 'wp_ajax_qppr_delete_all_qredirects', array( $this, 'qppr_delete_all_qreds_ajax' )  );		// register ajax delete ALL 1ON1 URL Redirects - 5.1.0
	...
	function qppr_load_textdomain() {
		load_plugin_textdomain( '1ON1_URL_Redirects', false, dirname( plugin_basename( __FILE__ ) ) . '/lang' );
	}
		...
		$p['qppr-meta-options'] = array(
			...
			'options' => array(
				'content' => sprintf( '<h3>%s</h3><p>%s</p>',
					__( 'New Meta Redirect options.' ,'1ON1_URL_Redirects'),
					__( 'Please view the Help Tab above to see more information about the Meta Redirect Settings.', '1ON1_URL_Redirects')
				),
		...
		$p['existing-redirects'] = array(
			...
			'options' => array(
				'content' => sprintf( '<h3>%s</h3><p>%s</p><p>%s</p>',
					__( 'New Layout of Existing Redirects' ,'1ON1_URL_Redirects'),
					__( 'The existing <strong>1ON1 URL Redirects</strong> are now laid out in a list format instead of form fields. When you have a lot of Redirects, this helps eliminate the "max_input_vars" configuration issue where redirects were not saving correctly.','1ON1_URL_Redirects'),
					__( 'To edit an existing redirect, click the pencil icon','1ON1_URL_Redirects'). ' (<span class="dashicons dashicons-edit"></span>) ' .__( 'and the row will become editable. Click the trash can icon','1ON1_URL_Redirects').' (<span class="dashicons dashicons-trash"></span>) '.__( 'and the redirect will be deleted. Click the trash can icon','1ON1_URL_Redirects')
				),
		...
		$p['qppr-use-jquery'] = array(
			...
			'options' => array(
				'content' => sprintf( '<h3>%s</h3><p>%s</p><p>%s</p><p>%s</p>',
					__( 'New Option to Use jQuery' ,'1ON1_URL_Redirects'),
					__( 'To increase the effectiveness of the plugin\'s ability to add new window and nofollow functionality, you can use the jQuery option.','1ON1_URL_Redirects'),
					__( 'This adds JavaScript/jQuery scripting to check the links in the output HTML of the page and add the correct functionality if needed.','1ON1_URL_Redirects'),
					__( 'If you experience JavaScript/jQuery conflicts, try turning this option off.','1ON1_URL_Redirects')
				),
	...
	function save_quick_redirects_fields(){
		if( isset( $_POST['submit_301'] ) ) {
			if( check_admin_referer( 'add_qppr_redirects' )){
				...
				$this->updatemsg 			= __( '1ON1 URL Redirects Updated.', '1ON1_URL_Redirects' );
				$this->qppr_try_to_clear_cache_plugins();
			}
	...
	function set_custom_edit_qppr_columns($columns) {
		$columns['qppr_redirect'] = __( 'Redirect', '1ON1_URL_Redirects' );
		return $columns;
	}
	...
	function ppr_add_menu_and_metaboxes(){
		/* add menus */
		 $icon_url=get_site_url().'/wp-content/plugins/1ON1_URL_Redirects/image/Logo-Picture-Large.png';
		$qppr_add_page 	= add_menu_page( '1ON1 URL Redirects', '1ON1 URL Redirects', 'manage_options', 'redirect-updates', array( $this, 'ppr_options_page' ),$icon_url );
		add_submenu_page( 'redirect-updates', '1ON1 URL Redirects', '1ON1 URL Redirects', 'manage_options', 'redirect-updates', array( $this,'ppr_options_page' ) );
		...
		add_submenu_page( 'redirect-updates', 'Tag Redirection setting', 'Tag Redirection setting', 'manage_options', 'tag_redirection_setting', array( $this, 'tag_redirection_setting' ) );
		...
		foreach( $post_types_temp as $type ){
			if( !in_array( $type, $ptypesNOTok ) ){
				...
				add_meta_box( 'edit-box-ppr', __( '1ON1 URL Redirects', '1ON1_URL_Redirects' ) , array( $this, 'edit_box_ppr_1' ), $type, $context, $priority );
			}
		}
	}
	
	/**************  custom code **************/
	function tag_redirection_setting(){
	if(isset($_POST['save_setting']) && ($_POST['save_setting']=="Save Changes")){
		$value=0;
		if(isset($_POST['enable_tab_redirect'])){
		$value=	$_POST['enable_tab_redirect'];
		}
		update_option('enable_tag_redirect',$value);
	}
	
	$status= get_option('enable_tag_redirect');
	...
	<div class="wrap">
	 ...
	 <input type="checkbox" name="enable_tab_redirect" <?php if($status==1){?> checked <?php } ?> value="1">
	 <strong>Enable Tag redirection </strong>
	 </div>
	 <div class="form-field">
	 <input type="submit" name="save_setting" value="Save Changes">
	 </div>
...
function redirect_tag_to_post() {
	...
	function qppr_admin_scripts($hook){
		// include admin js and css only for users who has access to Options (1ON1 URL Redirects admin pages)
		if ( current_user_can( 'manage_options' ) &&
				 in_array( $hook, array(
						'post-new.php',
						'edit.php',
			...
			wp_localize_script( 'qppr_admin_meta_script', 'qpprData', array(
				...
				'protocols' => $protocols,
				'msgAllDeleteConfirm' => __( 'Are you sure you want to PERMANENTLY Delete ALL Redirects and Settings (this cannot be undone)?', '1ON1_URL_Redirects' ),
				'msgQuickDeleteConfirm' => __( 'Are you sure you want to PERMANENTLY Delete ALL 1ON1 URL Redirects?', '1ON1_URL_Redirects' ),
				'msgIndividualDeleteConfirm' => __( 'Are you sure you want to PERMANENTLY Delets ALL Individual Redirects?', '1ON1_URL_Redirects' ),
				'msgDuplicate' => __( 'Redirect could not be saved as a redirect already exists with the same Request URL.', '1ON1_URL_Redirects' ) ,
				'msgDeleteConfirm' => __( 'Are you sure you want to delete this redirect?', '1ON1_URL_Redirects' ) ,
				'msgErrorSave' => __( 'Error Saving Redirect\nTry refreshing the page and trying again.', '1ON1_URL_Redirects' ) ,
				'msgSelect' => 'select a file',
				'msgFileType' => __( 'File type not allowed,\nAllowed file type: *.txt', '1ON1_URL_Redirects' ) ,
				'adminURL' => admin_url('admin.php'),
				'ajaxurl'=> admin_url('admin-ajax.php'),
				'error' => __('Please add at least one redirect before submitting form', '1ON1_URL_Redirects')
			));
		}
		return;
	}
	...
	function ppr_faq_page(){
		...
		echo '<a name="faq-top"></a><h2>'.__('Table of Contents','1ON1_URL_Redirects').'</h2>';
		echo '<ol class="qppr-faq-links">';
		echo implode( "\n", $linkfaq );
		echo '</ol>';
		echo '<h2>' . __( 'Questions/Answers', '1ON1_URL_Redirects' ) . '</h2>';
...
?>
<div class="wrap">
	<h2><?php echo __( '1ON1 URL Redirects Summary', '1ON1_URL_Redirects' );?></h2>
	<p><?php echo __( 'This is a summary of Individual &amp; Quick 301 Redirects.', '1ON1_URL_Redirects' );?></p>
	<br/>
	...
	<?php } ?>
	<?php $this->updatemsg ='';?>
	<h2 style="font-size:20px;"><?php echo __( 'Summary', '1ON1_URL_Redirects' );?></h2>
	<div align="left">
		<?php
			if($this->pproverride_active =='1'){echo '<div class="ppr-acor" style="margin:1px 0;width: 250px;font-weight: bold;padding: 2px;">' . __( 'Acitve Override is on - All Redirects are OFF!', '1ON1_URL_Redirects' ) . '</div>';}
			if($this->pproverride_nofollow =='1'){echo '<div class="ppr-nfor" style="margin:1px 0;width: 200px;font-weight: bold;padding: 2px;">' . __( 'No Follow Override is on!', '1ON1_URL_Redirects' ) . '</div>';}
			if($this->pproverride_newwin =='1'){echo '<div class="ppr-nwor" style="margin:1px 0;width: 200px;font-weight: bold;padding: 2px;">' . __( 'New Window Override is on!', '1ON1_URL_Redirects' ) . '</div>';}
			if($this->pproverride_rewrite =='1'){echo '<div class="ppr-rrlor" style="margin:1px 0;width: 200px;font-weight: bold;padding: 2px;">' . __( 'Rewrite Override is on!', '1ON1_URL_Redirects' ) . '</div>';}
			$labels 	= array(
				__( 'ID', '1ON1_URL_Redirects' ),
				__( 'post type', '1ON1_URL_Redirects' ),
				__( 'active', '1ON1_URL_Redirects' ),
				__( 'no follow', '1ON1_URL_Redirects' ),
				__( 'new window', '1ON1_URL_Redirects' ),
				__( 'type', '1ON1_URL_Redirects' ),
				__( 'rewrite link', '1ON1_URL_Redirects' ),
				__( 'original URL', '1ON1_URL_Redirects' ),
				__( 'redirect to URL', '1ON1_URL_Redirects' )
			);
			$labelsTD 	= array(
				'<span>'.$labels[0].' :</span>',
				'<span>'.$labels[1].' :</span>',
...
<?php
	...
	function ppr_import_export_page(){
		if(isset($_GET['update'])){
			if($_GET['update']=='4'){$this->updatemsg ='' . __( '1ON1 URL Redirects Imported & Replaced.', '1ON1_URL_Redirects' ) . '';}
			if($_GET['update']=='5'){$this->updatemsg ='' . __( '1ON1 URL Redirects Imported & Added to Existing Redirects.', '1ON1_URL_Redirects' ) . '';}
		}
		echo '<div class="wrap">';
		echo '	<h2>' . __( 'Import/Export Redirects', '1ON1_URL_Redirects' ) . '</h2>';
		if($this->updatemsg != '')
		...
		<div class="qppr-content">
			...
			<div class="qppr-left">
				...
				<tr valign="top">
					<td><label class="qppr-label"><strong><?php echo __( 'Export Redirects', '1ON1_URL_Redirects' );?></strong></label>
					<p><?php echo __( 'You should back-up your redirect regularly in case something happens to the database.', '1ON1_URL_Redirects' );?></p>
						<p><?php echo __( 'Please use the below buttons to make a back-up as either encoded (unreadable) or pipe separated', '1ON1_URL_Redirects' );?> (<code>|</code>).</p>
						<br /><p><input class="button button-primary qppr-export-quick-redirects" type="button" name="qppr-export-quick-redirects" value="<?php echo __( 'EXPORT all 1ON1 URL Redirects (Encoded)', '1ON1_URL_Redirects' );?>" onclick="document.location='<?php echo wp_nonce_url( admin_url('admin.php?page=redirect-options&qppr-file-type=encoded').'&action=export-quick-redirects-file', 'export-redirects-qppr'); ?>';" /></p>
						<p><?php echo __( 'OR', '1ON1_URL_Redirects' );?></p>
						<p><input class="button button-primary qppr-export-quick-redirects" type="button" name="qppr-export-quick-redirects" value="<?php echo __( 'EXPORT all 1ON1 URL Redirects (PIPE Separated)', '1ON1_URL_Redirects' );?>" onclick="document.location='<?php echo wp_nonce_url( admin_url('admin.php?page=redirect-options').'&action=export-quick-redirects-file&qppr-file-type=pipe', 'export-redirects-qppr'); ?>';" /></p>
					</td>
				</tr>
				<tr valign="top">
				...
				<tr valign="top">
					<td><label class="qppr-label"><strong><?php echo __( 'Import Redirects', '1ON1_URL_Redirects' );?></strong></label>
					<p><?php echo __( 'If you want to replace or restore redirects from a file, use the "Restore" option.', '1ON1_URL_Redirects' );?></p>
					<p><?php echo __( 'To add new redirects in bulk use the "Add To" option - NOTE: to Add To redirects, the file must be pipe dilimited ', '1ON1_URL_Redirects' );?> (<code>|</code>).</p>
						<br/>
						<input class="button-primary qppr-import-quick-redirects" type="button" id="qppr-import-quick-redirects-button" name="qppr-import-quick-redirects" value="<?php echo __( 'RESTORE Saved 1ON1 URL Redirects', '1ON1_URL_Redirects' );?>" />
						<?php echo __( 'OR', '1ON1_URL_Redirects' );?>
						<input class="button-primary qppr_addto_qr" type="button" id="qppr_addto_qr_button" name="qppr_addto_qr" value="<?php echo __( 'ADD TO 1ON1 URL Redirects', '1ON1_URL_Redirects' );?>" />
						<div id="qppr_import_form" class="hide-if-js2">
							<form action="<?php echo admin_url('admin.php?page=redirect-import-export'); ?>" method="post" enctype="multipart/form-data">
								<p style="margin:1em 0;">
									<label><?php echo __( 'Select 1ON1 URL Redirects file to import:', '1ON1_URL_Redirects' );?></label>
									<input type="file" name="qppr_file" onchange="qppr_check_file(this);" />
								</p>
								<p class="submit">
									<?php wp_nonce_field( 'import-quick-redrects-file' ); ?>
									<input class="button button-primary" type="submit" id="import-quick-redrects-file" name="import-quick-redrects-file" value="IMPORT & REPLACE Current 1ON1 URL Redirects" />
								</p>
							</form>
						</div>
						<div id="qppr_addto_form" class="hide-if-js">
							<form action="<?php echo admin_url('admin.php?page=redirect-import-export'); ?>" method="post" enctype="multipart/form-data">
								<p style="margin:.5em 0 1em 1em;color:#444;"> <?php echo __( 'The import file should be a text file with one rediect per line, PIPE separated, in this format:', '1ON1_URL_Redirects' );?><br/>
									<br/>
									<code><?php echo __( 'redirect|destination|newwindow|nofollow', '1ON1_URL_Redirects' );?></code><br/>
									<br/><?php echo __( 'for Example:', '1ON1_URL_Redirects' );?>
									<br/><br/>
									<code><?php echo __( '/old-location.htm|http://some.com/new-destination/|0|1', '1ON1_URL_Redirects' );?></code><br />
									<code><?php echo __( '/dontate/|http://example.com/destination/|1|1', '1ON1_URL_Redirects' );?></code><br/>
									<br/>
									<strong><?php echo __( 'IMPORTANT:', '1ON1_URL_Redirects' );?></strong> <?php echo __( 'Make Sure any destination URLs that have a PIPE in the querystring data are URL encoded before adding them!', '1ON1_URL_Redirects' );?><br/>
									<br/>
									<label><?php echo __( 'Select 1ON1 URL Redirects file to import:', '1ON1_URL_Redirects' );?></label>
								...
								<p class="submit">
									...
									<input class="button button-primary" type="submit" id="import_redirects_add_qppr" name="import_redirects_add_qppr" value="<?php echo __( 'ADD TO Current 1ON1 URL Redirects', '1ON1_URL_Redirects' );?>" />
								</p>
							</form>
						</div></td>
				</tr>
	...
	function ppr_settings_page() {
		if( isset( $_GET['update'] ) && $_GET['update'] != '' ){
			if( $_GET['update'] == '3' ){ $this->updatemsg = __( 'All 1ON1 URL Redirects deleted from database.', '1ON1_URL_Redirects' );}
			if( $_GET['update'] == '2' ){ $this->updatemsg = __( 'All Individual Redirects deleted from database.', '1ON1_URL_Redirects' );}
			if( $_GET['update'] == '4' ){ $this->updatemsg = __( '1ON1 URL Redirects Imported & Replaced.', '1ON1_URL_Redirects' );}
			if( $_GET['update'] == '5' ){ $this->updatemsg = __( '1ON1 URL Redirects Imported & Added to Existing Redirects.', '1ON1_URL_Redirects' );}
			if( $_GET['update'] == '6' ){ $this->updatemsg = __( 'All Redirects and Settings deleted from database', '1ON1_URL_Redirects' );}
			if( $_GET['update'] == '0' ){ $this->updatemsg = __( 'There was an problem with your last request. Please reload the page and try again.', '1ON1_URL_Redirects' );}
		}
	?>
<div class="wrap" style="position:relative;">
	<h2><?php echo __( '1ON1 URL Redirects Options', '1ON1_URL_Redirects' );?></h2>
	<?php if($this->updatemsg != ''){?>
	...
	<div class="qppr-content">
		...
		<form method="post" action="options.php" class="qpprform">
			...
			<table class="form-table">
				...
				<tr>
					<th scope="row" colspan="2"><h2 style="display:inline-block;"><?php echo __( 'Basic Settings', '1ON1_URL_Redirects' );?></h2></th>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Use with Custom Post Types?', '1ON1_URL_Redirects' );?></label></th>
				...
				<tr>
					<th scope="row"><label><span style="color:#FF0000;font-weight:bold;font-size:100%;margin-left:0px;"><?php echo __( 'Hide', '1ON1_URL_Redirects' );?></span> <?php echo __( 'meta box for following Post Types:', '1ON1_URL_Redirects' );?></label></th>
					<td><?php
				...
				<tr>
					<th scope="row"><label><?php echo __( 'Show Column Headers?', '1ON1_URL_Redirects' );?></label></th>
					<td><input type="checkbox" id ="ppr_show-columns" name="ppr_show-columns" value="1"<?php if(get_option('ppr_show-columns')=='1'){echo ' checked="checked" ';} ?>/> <span><?php echo __( 'Show Columns on list pages for set up redirects.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>

					<th scope="row"><label><?php echo __( 'Use jQuery?', '1ON1_URL_Redirects' );?></label></th>
					<td><input type="checkbox" id ="ppr_use-jquery" name="ppr_use-jquery" value="1"<?php if(get_option('ppr_use-jquery')=='1'){echo ' checked="checked" ';} ?>/> <span><?php echo __( 'Increases effectiveness of plugin. If you have a jQuery conflict, try turning this off.', '1ON1_URL_Redirects' );?></span><br /><span style="margin:0;"><?php echo __( 'Uses jQuery to add the "New Window" and "No Follow" attributes to links.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row" colspan="2" class="qppr-no-padding"><hr noshade color="#EAEAEA" size="1"></th>
				</tr>
				<tr>
					<th scope="row" colspan="2"><h2 style="font-size:20px;display:inline-block;"><?php echo __( 'Master Override Options', '1ON1_URL_Redirects' );?></h2><span><?php echo __( '<strong>NOTE: </strong>The below settings will override all individual settings.', '1ON1_URL_Redirects' );?></span></th>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Turn OFF all Redirects?', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="checkbox" name="ppr_override-active" value="1"<?php if(get_option('ppr_override-active')=='1'){echo ' checked="checked" ';} ?>/> <span><?php echo __( 'Basically the same as having no redirects set up.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Make ALL Redirects have <code>rel="nofollow"</code>?', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="checkbox" name="ppr_override-nofollow" value="1"<?php if(get_option('ppr_override-nofollow')=='1'){echo ' checked="checked" ';} ?>/> <span><?php echo __( 'Requires "use jQuery" option to work with 1ON1 URL Redirects.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Make ALL Redirects open in a New Window?', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="checkbox" name="ppr_override-newwindow" value="1"<?php if(get_option('ppr_override-newwindow')=='1'){echo ' checked="checked" ';} ?>/>	<span><?php echo __( 'Requires "use jQuery" option to work with 1ON1 URL Redirects.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Make ALL Redirects this type:', '1ON1_URL_Redirects' );?> </label></th>
					<td><select name="ppr_override-redirect-type">
							<option value="0"><?php echo __( 'Use Individual Settings', '1ON1_URL_Redirects' );?></option>
							<option value="301" <?php if( get_option('ppr_override-redirect-type')=='301') {echo ' selected="selected" ';} ?>>301 <?php echo __( 'Permanant Redirect', '1ON1_URL_Redirects' );?></option>
							<option value="302" <?php if( get_option('ppr_override-redirect-type')=='302') {echo ' selected="selected" ';} ?>>302 <?php echo __( 'Temporary Redirect', '1ON1_URL_Redirects' );?></option>
							<option value="307" <?php if( get_option('ppr_override-redirect-type')=='307') {echo ' selected="selected" ';} ?>>307 <?php echo __( 'Temporary Redirect', '1ON1_URL_Redirects' );?></option>
							<option value="meta" <?php if(get_option('ppr_override-redirect-type')=='meta'){echo ' selected="selected" ';} ?>><?php echo __( 'Meta Refresh Redirect', '1ON1_URL_Redirects' );?></option>
						</select>
						<span> <?php echo __( '(This will also override 1ON1 URL Redirects)', '1ON1_URL_Redirects' );?></span>
					</td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Make ALL redirects Case Sensitive?', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="checkbox" name="ppr_override-casesensitive" value="1"<?php if(get_option('ppr_override-casesensitive')=='1'){echo ' checked="checked" ';} ?>/> <span> <?php echo __( 'Makes URLs CaSe SensiTivE - i.e., /somepage/ DOES NOT EQUAL /SoMEpaGe/', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Make ALL Redirects go to this URL:', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="text" size="50" name="ppr_override-URL" value="<?php echo get_option('ppr_override-URL'); ?>"/> <span><?php echo __( 'Use full URL including <code>http://</code>.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Rewrite ALL Redirects URLs to Show in LINK?', '1ON1_URL_Redirects' );?> </label></th>
					<td><input type="checkbox" name="ppr_override-rewrite" value="1"<?php if(get_option('ppr_override-rewrite')=='1'){echo ' checked="checked" ';} ?>/> <span><?php echo __( 'Makes link show redirect URL instead of the original URL. Will only work on 1ON1 URL Redirects if the "Use jQuery" option is set.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row" colspan="2"><hr noshade color="#EAEAEA" size="1"></th>
				</tr>
				<tr>
					<th scope="row" colspan="2" class="qppr-no-padding"><h2 style="display:inline-block;"><?php echo __( 'Plugin Clean Up', '1ON1_URL_Redirects' );?></h2><span><?php echo __( '<strong>NOTE: </strong>This will DELETE all redirects - so be careful with this.', '1ON1_URL_Redirects' );?></span></th>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Delete Redirects?', '1ON1_URL_Redirects' );?> </label></th>
					<td>
						<input class="button-secondary qppr-delete-regular" type="button" name="qppr-delete-regular" value="<?php echo __( 'Delete All Individual Redirects', '1ON1_URL_Redirects' );?>" />
						<input class="button-secondary qppr-delete-quick" type="button" name="qppr-delete-quick" value="<?php echo __( 'Delete all 1ON1 URL Redirects', '1ON1_URL_Redirects' );?>" />
						<span style="display: block;margin-top: 5px;"><?php echo __( 'Individual Redirects are redirects set up on individual pages or posts when in the editing screen. The 1ON1 URL Redirects are set up on the 1ON1 URL Redirects page.', '1ON1_URL_Redirects' );?></span>
					</td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Delete ALL Redirects & Settings?', '1ON1_URL_Redirects' );?> </label></th>
					<td>
						<input class="button-secondary qppr-delete-everything" type="button" name="qppr-delete-everything" value="<?php echo __( 'Delete ALL Redirects AND Settings', '1ON1_URL_Redirects' );?>" />
						<span style="color: #0000ff;display: block;margin-top: 5px;"><?php echo __( 'All Redirects and Settings will be removed from the database. This can NOT be undone!', '1ON1_URL_Redirects' );?></span>
					</td>
				</tr>
			</table>
			<p class="submit"><input type="submit" class="button-primary" value="<?php echo __( 'Save Changes', '1ON1_URL_Redirects' );?>" /></p>
		</form>
		</div>
		<div class="clear-both"></div>
	</div>
	...
	function qppr_options_help_tab(){
		...
		if($screen_id == 'toplevel_page_redirect-updates' ){
			...
			$screen->add_help_tab( array(
				 ...
				 'title' => __( 'Examples', '1ON1_URL_Redirects' ),
				 'content' => $content ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr_add_redirects',
				 'title' => __( 'Troubleshooting', '1ON1_URL_Redirects' ),
				 'content' => '
				 <div style="padding:10px 0;">
				<b style="color:red;">' . __( 'IMPORTANT TROUBLESHOOTING NOTES:', '1ON1_URL_Redirects' ) . '</b>
				<ol style="margin-top:5px;">
					<li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'At this time the New Window (NW) and No Follow (NF) features will not work for 1ON1 URL Redirects unless "Use jQuery" is enabled in the options.', '1ON1_URL_Redirects' ) . '</li>
					<li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'It is recommended that the <b>Request URL</b> be relative to the ROOT directory and contain the <code>/</code> at the beginning.', '1ON1_URL_Redirects' ) . '</li>
					<li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'If you do use the domain name in the Request URL field, make sure it matches your site\'s domain style and protocol. For example, if your site uses "www" in front of your domain name, be sure to include it. if your site uses <code>https://</code>, use it as the protocol. Our best guess is that your domain and protocol are', '1ON1_URL_Redirects' ) . ' <code>'.network_site_url('/').'</code></li>
					<!--li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'If you are having issues with the link not redirecting on a SSL site with mixed SSL (meaning links can be either SSL or non SSL), try adding two redirects, one with and one without the SSL protocol.', '1ON1_URL_Redirects' ) . '</li-->
					<li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'The <b>Destination</b> field can be any valid URL or relative path (from root), for example', '1ON1_URL_Redirects' ) . ' <code>http://www.mysite.com/destination-page/</code> OR <code>/destination-page/</code></li>
					<li style="color:#214070;margin-left:15px;list-style-type:disc;">' . __( 'In order for NW (open in a new window) or NF (rel="nofollow") options to work with 1ON1 URL Redirects, you need to have:', '1ON1_URL_Redirects' ) . '
						<ol>
							<li>' . __( '"Use jQuery?" option selected in the settings page', '1ON1_URL_Redirects' ) . '</li>
							<li>' . __( 'A link that uses the request url SOMEWHERE in your site page - i.e., in a menu, content, sidebar, etc.', '1ON1_URL_Redirects' ) . ' </li>
							<li>' . __( 'The open in a new window or nofollow settings will not happen if someone just types the old link in the URL or if they come from a bookmark or link outside your site - in essence, there needs to be a link that they click on in your site so that the jQuery script can add the appropriate <code>target</code> and <code>rel</code> properties to the link to make it work.', '1ON1_URL_Redirects' ) . '</li>
						</ol>
					</li>
				</ol>
				</div>' ,
			...
			$screen->add_help_tab( array(
				 ...
				 'title' => __( 'Export Redirects', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'You can export redirects in two formats - Encoded or Delimited.', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr_import_redirects',
				 'title' => __( 'Import Redirects', '1ON1_URL_Redirects' ),
			...
			$screen->add_help_tab( array(
				 ...
				 'title' => __( 'Load Content?', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'Use the <strong>Load Content?</strong> option to allow the page content to load as normal or to only load a blank page or the content provided in the <strong>Page Content</strong> section. ', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'If checked, all of the original content will load, so keep this in mind when setting the <strong>Redirect Seconds</strong> - if set too low, the page will not compeletely load. ', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr-redirect-seconds',
				 'title' => __( 'Redirect Seconds', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'Enter the nuber of seconds to wait before the redirect happens. Enter 0 to have an instant redirect*.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( '*Keep in mind that the redirect seconds will start counting only AFTER the <strong>Redirect Trigger</strong> element is loaded - so 0 may be slightly longer than instant, depending on how much content needs to load before the trigger happens.', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr-redirect-trigger',
				 'title' => __( 'Redirect Trigger', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'The class or id or tag name of the element to load before the redirect starts counting down. If nothing is used, it will default to the body tag as a trigger.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'If you use a class, the class name should have the "." in the name, i.e., <strong>.my-class-name</strong>', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'If you use an id, the id should have the "#" in the name, i.e., <strong>#my-id-name</strong>.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'If you use a tag name, the name should NOT have the "&lt;" or "&gt;" characters in the name, i.e., &lt;body&gt; would just be <strong>body</strong>.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'Do not use a tag name that is common, like "a" or "div" as it will trigger on all events.', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr-redirect-append',
				 'title' => __( 'Append Content To', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'The class, id or tag name that you want the content in the <strong>Page Content</strong> to be loading into.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'If you are loading the content of the page, use an existing class or id for an existing element (i.e., .page-content) so your additional page content (if any) is loaded into that element.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'When no class, id or tag name is used, the <strong>body</strong> tag will be used.', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
			$screen->add_help_tab( array(
				 'id' => 'qppr-redirect-content',
				 'title' => __( 'Page Content', '1ON1_URL_Redirects' ),
				 'content' => '<div style="padding:10px 0;"><p>' . __( 'This is your page content you want to add. If you have a "tracking pixel" script or image tag you want to use, add it here.', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'A good example of use, is adding a tracking script (or Facebook Conversion Pixel) to the <strong>Page Content box</strong> and unchecking the <strong>Load Content?</strong> box. Then set the <strong>Redirect Seconds</strong> to 1 or 2 so the script has a chance to load and set <strong>Append Content</strong> To to "body" and <strong>Redirect Trigger</strong> to "body".', '1ON1_URL_Redirects' ) . '</p>
				 <p>' . __( 'Additionally, you can add the redirect counter to the page by adding the code sample under the <strong>Page Content</strong> box.', '1ON1_URL_Redirects' ) . '</p></div>' ,
			) );
		}
	}
...
?>
<div class="wrap">
	<h2><?php echo __( '1ON1 URL Redirects (301 Redirects)', '1ON1_URL_Redirects' );?></h2>
	<?php if($this->updatemsg != ''){?>
	...
	$isJQueryMsgHidden2 = get_option('qppr_jQuery_hide_message2');?>
		<?php if( $isJQueryOn == '' && ( $isJQueryMsgHidden == '' || $isJQueryMsgHidden == '0' ) ){ ?>
			<div class="usejqpprmessage error below-h2" id="usejqpprmessage">
				<?php echo __( 'The <code>Use jQuery?</code> option is turned off in the settings.<br/>In order to use <strong>NW</strong> (open in a new window) or <strong>NF</strong> (add rel="nofollow") options for 1ON1 URL Redirects, you must have it enabled.', '1ON1_URL_Redirects' );?><br/>
				<div class="hidepprjqmessage" style=""><a href="javascript:void(0);" id="hidepprjqmessage"><?php echo __( 'hide this message', '1ON1_URL_Redirects' );?></a></div>
			</div>
		<?php }elseif($isJQueryMsgHidden2 !='1'){ ?>
			<div class="usejqpprmessage info below-h2" id="usejqpprmessage2">
				<?php echo __( 'To use the <strong>NW</strong> (open in a new window) <strong>NF</strong> (nofollow) options, check the appropriate option and update when adding redirects. Then, any link in the page that has the request URL will be updated with these options (as long as you have <code>Use jQuery?</code> enabled in the plugin settings.', '1ON1_URL_Redirects' );?>
				<div class="hidepprjqmessage" style=""><a href="javascript:void(0);" id="hidepprjqmessage2"><?php echo __( 'hide this message', '1ON1_URL_Redirects' );?></a></div>
			</div>
		<?php }?>
	<p><?php echo __( '1ON1 URL Redirects are useful when you have links from an old site that now come up 404 Not Found, and you need to have them redirect to a new location on the current site - as long as the old site and the current site have the same domain name. They are also helpful if you have an existing URL that you need to send some place else and you don\'t want to create a Page or Post just to use the individual Page/Post Redirect option.', '1ON1_URL_Redirects' );?></p>
	<p><?php echo __( 'To add 1ON1 URL Redirects, put the URL for the redirect in the <strong>Request URL</strong> field, and the URL it should be redirected to in the <strong>Destination URL</strong> field. To delete a redirect, click the trash can at the end of that row. To edit a redirect, click the pencil edit icon.', '1ON1_URL_Redirects' );?></p>
	<p><?php echo __( 'See \'HELP\' in the upper right corner, for troubleshooting problems and example redirects.', '1ON1_URL_Redirects' );?></p>
	<form method="post" action="admin.php?page=redirect-updates" id="qppr_quick_save_form">
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tr>
				<th align="left" colspan="8"><h3><?php echo __( 'Add New Redirects', '1ON1_URL_Redirects' );?></h3></th>
			</tr>
			<tr>
				<th align="left" colspan="2"><?php echo __( 'Request URL', '1ON1_URL_Redirects' );?></th>
				<th align="left">&nbsp;</th>
				<th align="left"><?php echo __( 'Destination URL', '1ON1_URL_Redirects' );?></th>
				<th align="center"><?php echo __( 'NW', '1ON1_URL_Redirects' );?>*</th>
				<th align="center"><?php echo __( 'NF', '1ON1_URL_Redirects' );?>*</th>
				<th align="left"></th>
				<th align="left"></th>
			</tr>
			<tr>
				...
				<td class="table-qppr-nwn"><input class="pprnewwin" type="checkbox" name="quickppr_redirects[newwindow][0]" value="1" title="<?php echo __( 'open in a New Window', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-nfl"><input class="pprnofoll" type="checkbox" name="quickppr_redirects[nofollow][0]" value="1" title="<?php echo __( 'add No Follow', '1ON1_URL_Redirects' );?>" /></td>
			...
			<tr>
				...
				<td class="table-qppr-arr">&nbsp;&raquo;&nbsp;</td>
				<td class="table-qppr-des"><input type="text" name="quickppr_redirects[destination][]" value="" style="max-width:100%;width:100%;" /></td>
				<td class="table-qppr-nwn"><input class="pprnewwin" type="checkbox" name="quickppr_redirects[newwindow][1]" value="1" title="<?php echo __( 'open in a New Window', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-nfl"><input class="pprnofoll" type="checkbox" name="quickppr_redirects[nofollow][1]" value="1" title="<?php echo __( 'add No Follow', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-edt"></td>
				<td class="table-qppr-del"></td>
			</tr>
			<tr>
				...
				<td class="table-qppr-arr">&nbsp;&raquo;&nbsp;</td>
				<td class="table-qppr-des"><input type="text" name="quickppr_redirects[destination][]" value="" style="max-width:100%;width:100%;" /></td>
				<td class="table-qppr-nwn"><input class="pprnewwin" type="checkbox" name="quickppr_redirects[newwindow][2]" value="1" title="<?php echo __( 'open in a New Window', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-nfl"><input class="pprnofoll" type="checkbox" name="quickppr_redirects[nofollow][2]" value="1" title="<?php echo __( 'add No Follow', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-edt"></td>
				<td class="table-qppr-del"></td>
			</tr>
			<tr>
				<td style="text-align:right;" colspan="6"><div style="font-size: 11px;"><em>*<?php echo __( 'New Window(NW) and NoFollow(NF) functionality not available unless "Use with jQuery" is set in the options.', '1ON1_URL_Redirects' );?></em></div></td>
				<td style="text-align:right;" colspan="2"></td>
			</tr>
			<tr>
				<td align="left" colspan="8"><p class="submit"><input type="submit" name="submit_301" class="button button-primary" value="<?php echo __( 'Add New Redirects', '1ON1_URL_Redirects' );?>" /></p></td>
			</tr>
			<tr>
				<td class="newdiv" colspan="8"><div></div></td>
			</tr>
			<tr>
				<th align="left" colspan="8"><h3 id="qppr-existing-redirects"><?php echo __( 'Existing Redirects', '1ON1_URL_Redirects' );?></h3></th>
			</tr>
			<tr>
				<th align="left" colspan="2"><?php echo __( 'Request URL', '1ON1_URL_Redirects' );?></th>
				<th align="left">&nbsp;</th>
				...
				<th align="center"><?php echo __( 'NW', '1ON1_URL_Redirects' );?></th>
				<th align="center"><?php echo __( 'NF', '1ON1_URL_Redirects' );?></th>
				<th align="center"><?php //echo __( 'Edit', '1ON1_URL_Redirects' );?></th>
				<th align="center"><?php //echo __( 'Delete', '1ON1_URL_Redirects' );?></th>
			</tr>
			...
			<tr id="qppr-edit-row-holder" class="qppr-editing">
				...
				<td class="table-qppr-nwn cloned"><input class="input-qppr-neww" type="checkbox" name="newwindow" value="1" title="<?php echo __( 'open in a New Window', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-nfl cloned"><input class="input-qppr-nofo" type="checkbox" name="nofollow" value="1" title="<?php echo __( 'add No Follow', '1ON1_URL_Redirects' );?>" /></td>
				<td class="table-qppr-sav cloned"><span class="qpprfont-save" data-rowid="" title="<?php echo __( 'Save', '1ON1_URL_Redirects' );?>"></span></td>
				<td class="table-qppr-can cloned"><span class="qpprfont-cancel" data-rowid="" title="<?php echo __( 'Cancel', '1ON1_URL_Redirects' );?>"></span></td>
			</tr>
...
<?php
		...
		if (!empty($this->quickppr_redirects)) {
			...
			foreach ($this->quickppr_redirects as $request => $destination) {
				...
				<tr id="rowpprdel-'.$ww.'" class="qppr-existing">
					...
					<td class="table-qppr-edt"><span id="ppredit-'.$ww.'" class="edit-qppr dashicons-edit" data-rowid="rowpprdel-'.$ww.'" title="' . __( 'Edit', '1ON1_URL_Redirects' ) . '"></span></td>
					<td class="table-qppr-del"><span id="pprdel-'.$ww.'" class="delete-qppr dashicons-trash" data-rowid="rowpprdel-'.$ww.'" title="' . __( 'Delete', '1ON1_URL_Redirects' ) . '"></span></td>
				</tr>
				';
				$ww++;
			}
				...
				$output .= '
				<tr >
					<td colspan="8">' . __( 'No 1ON1 URL Redirects.', '1ON1_URL_Redirects' ) . '</td>
				</tr>
				';
		}
		return $output;
		...
		if( current_user_can( 'manage_options' ) ){
			if ( isset( $_GET['action'] ) && $_GET['action'] == 'export-quick-redirects-file' ) {
				...
				if ( $_FILES['qppr_file']['error'] > 0 ) {
					wp_die( __( 'An error occured during the file upload. Please fix your server configuration and retry.', '1ON1_URL_Redirects' ) , __( 'SERVER ERROR - Could Not Load', '1ON1_URL_Redirects' ), array( 'response' => '200', 'back_link' => '1' ) );
					exit;
				} else {
					...
					if ( substr($config_file, 0, strlen('QUICKPAGEPOSTREDIRECT')) !== 'QUICKPAGEPOSTREDIRECT' ) {
						...
						if($delim != false){
							...
							wp_die( __( 'This does not look like a 1ON1 URL Redirects file - it is possibly damaged or corrupt.', '1ON1_URL_Redirects' ) , __( 'ERROR - Not a valid File', '1ON1_URL_Redirects' ), array( 'response' => '200', 'back_link' => '1' ) );
							exit;
						}
					} else {
				...
				if ( $_FILES['qppr_file_add']['error'] > 0 ) {
					wp_die(__( 'An error occured during the file upload. It might me that the file is too large or you do not have the premissions to write to the temporary upload directory. Please fix your server configuration and retry.', '1ON1_URL_Redirects' ) , __( 'SERVER ERROR - Could Not Load', '1ON1_URL_Redirects' ), array( 'response' => '200', 'back_link' => '1' ) );
					exit;
				} else {
					...
					if ( strpos( $config_file, $delim ) === false ) {
						wp_die( __( 'This does not look like the file is in the correct format - it is possibly damaged or corrupt.<br/>Be sure the redirects are 1 per line and the redirect and destination are seperated by a PIPE (|), COMMA (,) or a TAB.', '1ON1_URL_Redirects' ) . '<br/>Example:<br/><br/><code>redirect|destination</code>', __( 'ERROR - Not a valid File', '1ON1_URL_Redirects' ), array( 'response' => '200', 'back_link' => '1' ) );
						exit;
					} else {
						$tempArr	 = array();
						$tempMArr	 = array();
						...
						if( !empty( $QR_Array ) && is_array( $QR_Array )):
							...
							wp_die( __( 'It does not look like there are any valid items to import - check the file and try again.', '1ON1_URL_Redirects' ) , __( 'ERROR - No Valid items to add.', '1ON1_URL_Redirects' ), array( 'response' => '200', 'back_link' => '1' ) );
							exit;
						endif;
					}
				}
	...
	function ppr_filter_plugin_actions($links){
		//$links[] 	= '<a href="'.$this->adminlink.'admin.php?page=redirect-options"><span class="dashicons dashicons-admin-settings"></span> ' . __( 'Settings', '1ON1_URL_Redirects' ) . '</a>';
		return $links;
	}

	function ppr_filter_plugin_links($links, $file){
		/* if ( $file == plugin_basename(__FILE__) ){
			$links[] = '<a href="'.$this->adminlink.'admin.php?page=redirect-updates"><span class="dashicons dashicons-external"></span> ' . __( '1ON1 URL Redirects', '1ON1_URL_Redirects' ) . '</a>';
			$links[] = '<a href="'.$this->adminlink.'admin.php?page=redirect-faqs"><span class="dashicons dashicons-editor-help"></span> ' . __( 'FAQ', '1ON1_URL_Redirects' ) . '</a>';
			$links[] = '<a target="_blank" href="'.$this->fcmlink.'/donations/"><span class="dashicons dashicons-heart"></span> ' . __( 'Donate', '1ON1_URL_Redirects' ) . '</a>';
		}*/
		return $links; 
	}
	...
	// Prints the inner fields for the custom post/page section
		...
		echo '<label for="pprredirect_active" style="padding:2px 0;"><input type="checkbox" name="pprredirect_active" value="1" '. checked('1',get_post_meta($post->ID,'_pprredirect_active',true),0).' />&nbsp;' . __( 'Make Redirect <strong>Active</strong>.', '1ON1_URL_Redirects' ) . '<span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'Check to turn on or redirect will not work.', '1ON1_URL_Redirects' ) . '</span></span></label><br />';
		echo '<label for="pprredirect_newwindow" style="padding:2px 0;"><input type="checkbox" name="pprredirect_newwindow" id="pprredirect_newwindow" value="_blank" '. checked('_blank',get_post_meta($post->ID,'_pprredirect_newwindow',true),0).'>&nbsp;' . __( 'Open in a <strong>new window.</strong>', '1ON1_URL_Redirects' ) . '<span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'To increase effectivness, select "Use jQuery" in the options.', '1ON1_URL_Redirects' ) . '</span></span></label><br />';
		echo '<label for="pprredirect_relnofollow" style="padding:2px 0;"><input type="checkbox" name="pprredirect_relnofollow" id="pprredirect_relnofollow" value="1" '. checked('1',get_post_meta($post->ID,'_pprredirect_relnofollow',true),0).'>&nbsp;' . __( 'Add <strong>rel="nofollow"</strong> to link.', '1ON1_URL_Redirects' ) . '<span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'To increase effectivness, select "Use jQuery" in the options.', '1ON1_URL_Redirects' ) . '</span></span></label><br />';
		echo '<label for="pprredirect_rewritelink" style="padding:2px 0;"><input type="checkbox" name="pprredirect_rewritelink" id="pprredirect_rewritelink" value="1" '. checked('1',get_post_meta($post->ID,'_pprredirect_rewritelink',true),0).'>&nbsp;' . __( '<strong>Show</strong> Redirect URL in link.', '1ON1_URL_Redirects' ) . ' <span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'To increase effectivness, select "Use jQuery" in the options. This will only change the URL in the link <strong>NOT</strong> the URL in the Address bar.', '1ON1_URL_Redirects' ) . '</span></span></label><br /><br />';
		...
		echo '<label for="pprredirect_url"><b>' . __( 'Redirect / Destination URL:', '1ON1_URL_Redirects' ) . '</b></label><br />';
		echo '<input type="text" style="width:75%;margin-top:2px;margin-bottom:2px;" name="pprredirect_url" value="'.$pprredirecturl.'" /><span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help"><br />' . __( '(i.e., <strong>http://example.com</strong> or <strong>/somepage/</strong> or <strong>p=15</strong> or <strong>155</strong>. Use <b>FULL URL</b> <i>including</i> <strong>http://</strong> for all external <i>and</i> meta redirects.)', '1ON1_URL_Redirects' ) . '</span></span><br /><br />';
		echo '<label for="pprredirect_type"><b>' . __( 'Type of Redirect:', '1ON1_URL_Redirects' ) . '</b></label><br />';
		...
		<option value="301" '.$ppr_option1.'>301 ' . __( 'Permanent', '1ON1_URL_Redirects' ) . '</option>
		<option value="302" '.$ppr_option2.'>302 ' . __( 'Temporary', '1ON1_URL_Redirects' ) . '</option>
		<option value="307" '.$ppr_option3.'>307 ' . __( 'Temporary', '1ON1_URL_Redirects' ) . '</option>
		<option value="meta" '.$ppr_option5.'>' . __( 'Meta Redirect', '1ON1_URL_Redirects' ) . '</option>
		</select><span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'Default is 301 (Permanent Redirect).', '1ON1_URL_Redirects' ) . ' </span></span><br /><br />
		';
		...
		echo '	<label for="pprredirect_meta_secs" style="padding:2px 0;"><strong>' . __( 'Redirect Seconds (ONLY for meta redirects).', '1ON1_URL_Redirects' ) . '</strong></label><br /><input type="text" name="pprredirect_meta_secs" id="pprredirect_meta_secs" value="'. (get_post_meta($post->ID,'_pprredirect_meta_secs',true) != '' ? get_post_meta($post->ID,'_pprredirect_meta_secs',true ): '' ).'" size="3"><span class="qppr_meta_help_wrap"><span class="qppr_meta_help_icon dashicons dashicons-editor-help"></span><span class="qppr_meta_help">' . __( 'Leave blank to use options setting. 0 = instant.', '1ON1_URL_Redirects' ) . ' </span></span><br /><br />';
		echo '</div>';
		echo __( '<strong>NOTE:</strong> For a Page or Post (or Custom Post) Redirect to work, it may need to be published first and then saved again as a Draft. If you do not already have a page/post created you can add a \'Quick\' redirect using the', '1ON1_URL_Redirects' ) . ' <a href="./admin.php?page=redirect-updates">' . __( '1ON1 URL Redirects', '1ON1_URL_Redirects' ) . '</a> ' . __( 'method.', '1ON1_URL_Redirects' );
	}
				...
				if($val) {
					...
					if( $useURL != '' ){
						...
						if( $qpprRedType == 'meta' ){
							...
							header('RedirectType: 1ON1 URL Redirects - Quick');
			...
			if( isset( $this->ppr_all_redir_array[$post->ID] ) ){
				...
				if($isactive == 1 && $redrurl != '' && $redrurl != 'http://www.example.com'){
					...
					if($redrtype == 'meta'){
						...
						header('RedirectType: 1ON1 URL Redirects - Individual');
	...
	?>
	<div class="wrap" style="position:relative;">
		<h2><?php echo __( 'Meta Redirect Settings', '1ON1_URL_Redirects' );?></h2>
		<?php if ( ! empty( $_GET['settings-updated'] ) ) : ?><div id="message" class="updated notice is-dismissible"><p><?php echo __( 'Settings Updated', '1ON1_URL_Redirects' );?></p></div><?php endif; ?>
		<p><?php echo __( 'This section is for updating options for redirects that use the "meta refresh" funcitonality for redirecting.', '1ON1_URL_Redirects' );?></p>
		<p><?php echo __( 'Using the setting below, you can add elements or a message to the page that is loaded before tht redirect, or just allow the page to load as normal until the redirect reaches the number of seconds you have set below.', '1ON1_URL_Redirects' );?></p>
		<form method="post" action="options.php" class="qpprform">
			<?php settings_fields( 'qppr-meta-settings-group' ); ?>
			<table class="form-table">
				<tr>
					<th scope="row"><label id="qppr-meta-options"><?php echo __( 'Load Page Content?', '1ON1_URL_Redirects' );?></label></th>
					<td><input type="checkbox" name="qppr_meta_addon_load" value="1" <?php echo ( ( get_option( 'qppr_meta_addon_load', '' ) != '' ) ? ' checked="checked"' : '' ); ?> /><span><?php echo __( 'Check if you want the normal page to load before redirect happens (if redirect is 0 seconds, it may not load fully).', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Redirect Seconds', '1ON1_URL_Redirects' );?>:</label></th>
					<td><input type="text" size="5" name="qppr_meta_addon_sec" value="<?php echo get_option('qppr_meta_addon_sec', '0'); ?>"/><span><code>0</code> = <?php echo __( 'instant', '1ON1_URL_Redirects' );?>*. <code>10</code> <?php echo __( 'would redirect 10 seconds after the required element is loaded (i.e., body or an element with a specific class). *Intsant will still have a \'slight\' delay, as some content needs to load before the redirect occurs. Settings on individual pages will override this setting.', '1ON1_URL_Redirects' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Redirect Trigger', '1ON1_URL_Redirects' );?>:</label></th>
					<td><input type="text" size="25" id="qppr_meta_addon_trigger" name="qppr_meta_addon_trigger" value="<?php echo get_option('qppr_meta_addon_trigger', 'body'); ?>"/><span><?php printf( __( 'The %1$s, %2$s or tag name of the element you want to load before triggering redirect. Use a %3$s in the class name or %4$s for the ID. <strong><em>For example:</em></strong> if you want it to redirect when the body tag loads, you would type %5$s above. To redirect after an element with a class or ID, use %6$s or %7$s.', '1ON1_URL_Redirects' ), '<code>class</code>', '<code>ID</code>', '<code>.</code>', '<code>#</code>', '<code>body</code>', '<code>.some-class</code>', '<code>#some-id</code>' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Append Content To', '1ON1_URL_Redirects' );?>:</label></th>
					<td><input type="text" size="25" id="qppr_meta_append_to" name="qppr_meta_append_to" value="<?php echo get_option('qppr_meta_append_to', 'body'); ?>"/><span><?php printf( __( 'The %1$s, %2$s or tag name of the element you want the content to load into when the page loads.', '1ON1_URL_Redirects' ), '<code>class</code>', '<code>ID</code>' );?></span></td>
				</tr>
				<tr>
					<th scope="row"><label><?php echo __( 'Page Content', '1ON1_URL_Redirects' );?>:</label></th>
					<td><span><?php printf( __( 'Be sure to include a tag with your class or ID or tag name (entered above) so the redirect triggers - if you do not, the redirect will not happen. If you check the box to "Load Page Content", this data will be inserted into the page right after the %1$s tag. Otherwise, it will be the only content shown.', '1ON1_URL_Redirects' ), '&lt;body&gt;');?><br /><strong><br /><?php echo __( 'Add your content below', '1ON1_URL_Redirects' );?></strong>.</span>
						<textarea id="qppr_meta_addon_content" name="qppr_meta_addon_content"><?php echo get_option('qppr_meta_addon_content', ''); ?></textarea>
						<br /><span><?php echo __( 'To use a counter, add the following:', '1ON1_URL_Redirects' );?>
						<pre>&lt;div id="qppr_meta_counter" data-meta-counter-text="This page will redirect in %1$ seconds."&gt;&lt;/div&gt;</pre>
						<?php echo __( 'The "%1$" will be replaced with the actual seconds.', '1ON1_URL_Redirects' );?>
						</span>
					</td>
				</tr>
			</table>
	...
	function qppr_meta_addon_admin_notice() {
		...
		<div class="update-nag">
			' . __( 'You have the Addon Plugin', '1ON1_URL_Redirects' ) . ' <strong>"QPPR - Meta Redirect Add On"</strong> ' . __( 'activated. This plugin\'s functionality is now built into the parent', '1ON1_URL_Redirects' ) . ' <strong>"1ON1 URL Redirects Plugin"</strong> ' . __( 'so you no longer need to have the addon plugin installed.', '1ON1_URL_Redirects' ) . '
			<br /><br />' . __( 'The plugin will be deactivated now to prevent conflicts. You may delete it if you desire.', '1ON1_URL_Redirects' ) . '
		</div>';
	}
}
//=======================================
